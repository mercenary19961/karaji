...
> **📍 Doc sync:** CLAUDE.md last synced to commit `1fdca59` — 2026-07-05 15:08 (Sun). Convention: whenever you edit this file, refresh this line to the current commit — run `git log -1 --format="%h %cd" --date=format:"%Y-%m-%d %H:%M (%a)"` and paste the hash + date + time.
...
- [ ] **Validate mockup with a real shop owner** → collect reactions per screen (esp. visit-entry speed + WhatsApp summary) before any schema work
